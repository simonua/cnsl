...
5. Add a concise, visitor-friendly summary of the accepted data changes to the undated `Upcoming` section in `src/views/whats-new.html`. Write at about an eighth-grade reading level with common words, short sentences, and no technical or source-review jargon. State what families can now see or rely on and include only enough detail to understand the change. Classify updater-originated annual-data changes with the existing `<span class="release-label release-label--data-update">Data update</span>` label; do not classify them as `Bug fix` unless the same item also corrects application behavior rather than published data. Within a material data review, this agent owns the initial data-change note and re-ranks the complete `Upcoming` list using the repository's category-first and visitor-impact rules: features first, bug fixes second, and data updates last, without category headings or sections. `version-update` later owns promotion into a stable release. Never edit, remove, or append content to a dated release entry. Update the What's New entry in both `runlocal` and `publish` modes whenever material application data changes, and update the page's `sitemap.xml` `lastmod` value to the actual content-change date.
