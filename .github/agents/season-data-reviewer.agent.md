...
Run `pnpm run check:data-updates` before opening broad annual files or official pages. If it reports no candidate differences, stop the source review without loading domain JSON or browsing sources. Report the active season, explain that the monitor checked the configured official pool, meet, and team source fingerprints and retained-document validators, and state that no repository files were changed. Then ask whether the user would like to run `pnpm run validate:data` as a separate, more comprehensive local integrity check of the active JSON, schemas, cross-domain references, source URLs, and retained PDF inventory. Be clear that this validation does not perform a broader remote-source search. Do not run it without the user's confirmation. When differences exist, use each report row's exact review scope to read only the named annual record and modeled properties, then retrieve only its listed official source. Do not scan unaffected records or domains. The monitor removes a stale generated report after a clean run so an earlier candidate cannot be mistaken for current evidence.
