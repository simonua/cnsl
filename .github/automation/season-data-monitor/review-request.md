...
Use the reviewer's default `runlocal` mode unless the appended request explicitly selects `publish`. Compare the changed official evidence with the active annual JSON fields represented by the application. After every completed review, update the last-checked timestamp and refresh the reviewed monitor baseline. If a modeled value or application-used source destination needs to change, update the data and any supporting retained official document together and also update the last-updated timestamp. Run the required verification locally. Do not create or switch branches, stage files, commit, push to `origin`, or open a pull request unless `publish` mode was explicitly requested.
