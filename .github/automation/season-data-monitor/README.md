...
The workflow uses the `CNSL_DATA_UPDATER_PR_TOKEN` repository secret to create an issue assigned to Copilot coding agent when a previously unseen candidate is confirmed. Configure a user token for a licensed user that can assign Copilot in this repository, with repository read/write access to `Actions`, `Contents`, `Issues`, and `Pull requests` plus the required metadata read permission. The built-in workflow token remains `contents: read` only for checkout.
