...
Add every completed online source review to the check log. Use the review-completion timestamp in `America/New_York` with an explicit UTC offset, record the active season and source counts, and state whether the overall check was complete. Keep detailed entries newest-first immediately below the `Latest Activity` table.

Whenever an online review, local correction, or seasonal-data maintenance workflow adds a detailed entry, also add one row to the top `Latest Activity` table. Keep the table and detailed entries in the same newest-first order. Give each detailed section an explicit, unique `activity-...` anchor immediately before its heading, and link the table's `Date` value to that anchor. Use `check` when the activity only reviews or verifies existing state. Use `update` when repository data, a data contract, a visitor-used destination or retained source, or visitor-facing content or behavior changes. In `Data point`, name the most specific modeled record or property changed; leave it visibly blank for checks and for maintenance that does not change modeled data by writing `<!-- -->` as the lint-compatible cell source. Use `Summary` for the general outcome, including clean or incomplete checks and non-data maintenance. Preserve prior entries; `update-report.md` remains an ephemeral candidate report and is not the historical audit log.
