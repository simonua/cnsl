...
- **Analytics must remain purpose-limited.** Use Google Analytics only for aggregate public page counts, reviewed fixed campaign labels on app-published inbound links, and coarse feature-use categories. Never send names, contact details, account identifiers, precise or saved location, saved settings values, selected pools or teams, selected filter values, free-form/user-provided text, arbitrary URL query/fragment values, referrers, or any app-authored data intended to identify or profile a visitor. Map approved campaign labels to GA campaign fields only after validating an allowlisted tuple and remove its query tags from the visible URL; do not transmit a full tagged page location. Deny analytics and advertising storage, Google signals, and advertising/personalization signals so campaign effectiveness is measured without app-enabled visitor identifiers or cross-visit analytics storage. Do not enable additional GA4 enhanced measurement or granular location/device reporting without a separate review against this boundary and regression tests.
