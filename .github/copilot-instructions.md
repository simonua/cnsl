...
- **Analytics must remain purpose-limited.** Use Google Analytics only for site-use and feature-use measurement. Never send names, contact details, account identifiers, precise or saved location, free-form/user-provided text, URL query/fragment values, referrers, or any application data intended to identify or profile a visitor. Any tracked app choice must be a fixed public option needed to understand feature use, documented in the privacy decision, and protected by tests. Analytics storage is enabled for standard GA4 user/session reporting; keep advertising storage, Google signals, and advertising/personalization signals disabled. Require GA4 administrative features such as enhanced measurement and granular location/device reporting to remain limited to fields separately audited against this boundary.
