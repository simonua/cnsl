...
- A single inventory row may cover an explicitly scoped PDF set only when the same download-date value or `Not recorded` status applies to every member and the README states that coverage; otherwise list retained documents individually. Pool inventory rows may use a recursive glob scoped to one download date when the row states how many pool artifacts it covers.
...
- Official PDF subfolders are domain-specific: `pools/pool-schedules/`, `meets/meet-schedules/`, and `teams/team-schedules/` when source PDFs exist. Store each pool PDF as `pools/pool-schedules/<pool-id>/<YYYY-MM-DD>/<official-filename>`, using the stable pool ID and actual download date. Add a new dated artifact when the publisher changes a PDF; never replace or delete an earlier retained version. Retain league-wide practice schedules and team-assignment PDFs under `teams/team-schedules/` when published.
...
- Pool facility `features` preserve the published baseline from the official Columbia Association page stored in each pool's `caUrl`. For every season rollover, review each page's description and Amenities list, normalize useful terms into feature labels, and record the verification date in the annual README instead of copying the prior year's array without review. Use `featureOverrides` only for an explicitly evidenced correction to that baseline, and derive the effective visitor-facing list without rewriting which values the page published.
...
- Keep concession `mealItems`, `snackItems`, and `drinkItems` arrays in case-insensitive A-to-Z order. Preserve the source wording while alphabetizing each category independently.
- Official public Columbia Association pages/documents are authoritative for pool data, and official public CNSL publication/team pages are authoritative for meet and team data. Do not accept social media or other community-shared information as data evidence unless it appears in an official public source. A dated direct observation by the repository maintainer may support a pool `featureOverrides` addition only for an objective, visible physical amenity after current relevant CA sources have been checked and none explicitly contradicts the observation. Record the observation date, official-source check date, neutral factual note, and any corroborating official URL. Direct observation must not remove a published feature or establish schedules, operating status, admission rules, accessibility/safety claims, or another fact that is not a plainly visible facility presence.
...
## Pool Schedule Harvesting

- Treat the complete current pool-specific schedule as the harvesting unit. For each stable pool ID, select the newest retained artifact that byte-matches or is otherwise verified against the current live official document; keep older dated artifacts as evidence history, not as separate current schedules.
- Extract PDFs with layout preservation by running `pnpm run extract:pdf -- <retained-pdf-path>`. Review the extracted text and the rendered PDF together whenever columns, wrapping, footnotes, asterisks, headings, or adjacent time ranges make ownership ambiguous. Never rely on concatenated text alone to assign a note or value.
- Compare every source schedule period and exception with the exact pool record. Account for period start and end dates, weekdays, ordered activity labels and combinations, start and end times, whole-slot `accessStatus`, exact lane counts for every named simultaneous use, and whether general swimming remains available.
- Harvest visitor-relevant qualifiers outside the main time grid. This includes partial-facility availability or closure such as a wading pool, splash pad, diving well, or assigned lanes; age or participation limits; explicit open-during-program notes; cancellations; season-end transitions; and other conditions that change what part of the facility a visitor can use.
- Inspect every dated footnote, holiday schedule, and one-date exception independently of recurring periods. A current pool-specific date exception may override a recurring program series when field ownership, specificity, and currency resolve the evidence at `High` confidence. Do not drop an exception merely because the recurring series ends earlier or lists a conflicting occurrence.
- Preserve event identity and scope exactly enough to avoid changing its meaning. Distinguish CNSL meets, school meets, team practices, village events, classes, parties, and other uses; record whether they occur before public hours, replace public hours, or share the facility. Do not infer the event owner from a generic `Swim Meet`, `Practice`, or closure label.
- Store supported planning detail in its semantic field first. When the current schema has no narrower field and the detail qualifies an existing slot, preserve concise source-backed context in `notes`; do not invent a new field. Record useful official material that cannot be represented faithfully in the annual README and classify whether it is non-material, intentionally unmodeled, or unresolved.
- Resolve every source annotation explicitly during a comprehensive audit. Mark it as modeled, accepted for correction, non-material to visitor planning, intentionally unmodeled, or unresolved; do not declare a pool current after comparing only dates, activities, and times. Generic promotions are normally non-material, while a reminder that the pool remains open during another activity may be non-material only when the modeled public interval and access already make that availability clear.
- For a comprehensive multi-pool audit, produce a per-pool matrix naming the current artifact, modeled result, accepted differences, confidence, and any unresolved clarification. Re-scan the extracted corpus for lane terms, facility-area terms, footnote markers, cancellation language, and event labels after the record-by-record comparison so repeated omission patterns are not missed.
...
- Treat a qualifying maintainer observation as `High` confidence only for the visible presence of the exact amenity observed, when it is dated, current official sources have been inspected, and no official source explicitly contradicts it. Official structured corroboration should be recorded when available. The observation does not imply that the amenity will be open or usable during every visit.
...
- Keep each `featureOverrides` entry unique by feature. An `add` must be absent from the published `features` baseline, a `remove` must be present there, and a removal requires official-source evidence. Do not copy feature overrides into a later season without renewed evidence.
