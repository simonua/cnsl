...
- Treat every changed JavaScript test or browser spec as executable JavaScript under the repository lint requirement. Run `pnpm exec eslint <exact changed test files>` during focused iteration, then run `pnpm run lint` before completion whenever any `tests/**/*.js` file changed; a passing unit or Playwright run does not replace this static check.
...
- Browser specs are linted in a Node environment even though callbacks passed to `page.evaluate`, `locator.evaluate`, and related Playwright APIs execute in the browser page. Inside those callbacks, access page globals explicitly through `globalThis`, such as `globalThis.document` or `globalThis.getComputedStyle`, or receive values through callback arguments. Do not expose browser globals across the whole test lint environment, because that would hide accidental browser-only references in Node-side test code.
...
- The unit-gated browser source contract rejects known mutable directory-loaded prose and `page.waitForTimeout()`. Extend that deterministic guard only for patterns with an unambiguous semantic replacement; do not ban exact accessibility names, fixture-owned copy, ordered collection checks, or presentation assertions when those are the behavior under test.
