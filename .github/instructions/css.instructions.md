...
- Keep ordinary stylesheet changes compatible with the shared Content Security Policy in `src/views/layouts/base.html`: same-origin `styles.css` and the exact initial-canvas hash are the complete style sources. Never add `'unsafe-inline'`, `style-src-attr`, a new style origin, or a broader source expression to make a CSS change work.
- Review every new CSS `url()`, `@import`, font, data URL, blob URL, or external asset against the applicable CSP directive and `docs/security-privacy.md`. Prefer an existing same-origin asset and do not broaden CSP without an explicit security and privacy decision plus focused artifact coverage.
