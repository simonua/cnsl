...
- Start-page analytics may report only a selection validated against the application-owned `StartPage.VALUES` allowlist. Favorite pool and team analytics may report one selection only after validating it against the currently published annual data, or the fixed `none` value when cleared. Pool feature filter analytics may report the sorted validated public feature names joined with `|`, or the fixed `none` value when cleared. Do not track themes, layouts, refresh values, location choices, other filter values, or any other selected settings; those settings may report the changed category only.
