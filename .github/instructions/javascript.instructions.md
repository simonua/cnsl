...
- Analytics may report public page counts, reviewed fixed campaign labels on app-published inbound links, coarse feature-use categories, annual-data-validated favorite pool and team selections, annual-data-validated pool feature filter selections, and annual-data-validated public pool identities for pool-specific interactions; public pool identities and feature names are directory facts rather than visitor-identifying data. Do not introduce account identifiers, contact data, coordinates, application-stored device values, other selected preference values, user-entered strings, arbitrary URL query/fragment content, or referrer data.
