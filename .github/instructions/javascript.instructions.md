...
- Prefer existing schema-shaped design-time typedefs from `types/`, runtime literal unions and enums from `src/js/types/`, DOM types, and named record shapes. Use an honest broad type such as `Object`, `Array`, or `*` when no stable narrower contract exists; do not invent undefined type names merely to appear precise.
...
- Analytics may report public page counts, reviewed fixed campaign labels on app-published inbound links, coarse feature-use categories, the allowlisted start-page selection, annual-data-validated favorite pool and team selections, annual-data-validated pool feature filter selections, annual-data-validated public pool identities for pool-specific interactions, and privacy-limited technical errors; public pool identities and feature names are directory facts rather than visitor-identifying data. Technical-error events may report only the published app version, an allowlisted standard error name, fixed runtime context, validated same-origin app script path or fixed source category, bounded line and column numbers, and a fingerprint derived only from those approved fields. Do not transmit raw error messages, rejection values, stack traces, external script URLs, account identifiers, contact data, coordinates, application-stored device values, selected preference values outside these reviewed exceptions, user-entered strings, arbitrary URL query/fragment content, or referrer data.
- Start-page analytics may report only a selection validated against the application-owned `StartPage.VALUES` allowlist. Location-awareness analytics may report only `enabled` or `disabled` after validation against the application-owned allowlist; never report coordinates, current or saved location, calculated distance, browser permission state, or permission results. Favorite pool and team analytics may report one selection only after validating it against the currently published annual data, or the fixed `none` value when cleared. Pool feature filter analytics may report the sorted validated public feature names joined with `|`, or the fixed `none` value when cleared. Do not track themes, layouts, refresh values, other location choices, other filter values, or any other selected settings; those settings may report the changed category only.
