...
- Analytics may report public page counts, reviewed fixed campaign labels on app-published inbound links, coarse feature-use categories, and annual-data-validated favorite pool and team selections only; do not introduce account identifiers, contact data, coordinates, application-stored device values, other selected preference values, user-entered strings, arbitrary URL query/fragment content, or referrer data.
