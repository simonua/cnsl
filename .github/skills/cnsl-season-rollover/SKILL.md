...
2. Read `docs/annual-season-assets.md`, inspect `src/assets/data/<YEAR>/`, and inspect `src/js/config/app-config.js` to learn whether the year is active or preparatory.
  For a latest-update check of the active season, run `pnpm run check:data-updates` first. If it reports no candidate differences, stop the source review without loading broad annual JSON or retrieving official sources. Report the active season, monitored source categories, and that no files changed, then ask whether the user wants the separate `pnpm run validate:data` integrity check. Explain that validation checks active JSON, schemas, cross-domain references, source URLs, and retained PDF inventory but is not a broader remote-source search; do not run it without confirmation. If the monitor reports differences, inspect only each finding's exact JSON review scope and listed source.
