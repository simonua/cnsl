...
7. Validate each completed JSON document against its sibling schema. Require ISO `YYYY-MM-DD` dates, maintain stable pool identifiers used by application links, confirm every pool has a reviewed `features` array sourced from its official CA page, and confirm every pool activity and combined activity array has an explicit matrix-backed access classification.
