...
9. Verify every pool's official Independence Day schedule. The active annual data must include a July 4 `replace-day` override for each pool with public hours matching `INDEPENDENCE_DAY_NOTICE_SCHEDULE` in `src/js/config/app-config.js`. Confirm `APP_ATTENTION_NOTICE` starts one week ahead, uses the `information` type, does not show update metadata, and expires when Independence Day ends. If current official evidence does not support uniform hours at every pool, update or disable the notice before activation and record the evidence and decision in the annual README.
10. Decide whether to activate the target year:
