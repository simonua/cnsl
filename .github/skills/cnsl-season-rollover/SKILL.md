...
2. Read `docs/annual-season-assets.md`, inspect `src/assets/data/<YEAR>/`, and inspect `src/js/config/app-config.js` to learn whether the year is active or preparatory.
  For a latest-update check of the active season, run `pnpm run check:data-updates` first. If it reports no candidate differences, stop without loading broad annual JSON or retrieving official sources. If it reports differences, inspect only each finding's exact JSON review scope and listed source.
