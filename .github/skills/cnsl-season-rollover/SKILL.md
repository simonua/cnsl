...
2. Read `docs/annual-season-assets.md`, inspect `src/assets/data/<YEAR>/`, and inspect `src/js/config/app-config.js` to learn whether the year is active or preparatory.
  For a latest-update check of the active season, run `pnpm run check:data-updates` first. This command must make live network requests during the current run; a prior report or local baseline is not a source check. Reconcile the monitor's collected source list with every official source defined by the active annual JSON and annual README for modeled data or an application-used official source destination. Directly issue `HEAD` or conditional `GET` requests for any defined source not collected by the monitor, falling back to `GET` content or document comparison when response validators are unavailable or unreliable. Treat `304 Not Modified` and unchanged current `ETag` or `Last-Modified` values as successful online checks. If any expected source was not attempted or failed, report the check as incomplete rather than clean. If all expected sources were reached and no candidate differences exist, stop without loading broad annual JSON or retrieving unchanged source bodies for AI review. Report the active season, checked source categories and counts, request coverage or failures, and that no files changed, then ask whether the user wants the separate `pnpm run validate:data` integrity check. Explain that validation checks active JSON, schemas, cross-domain references, URL syntax, and retained PDF inventory locally; it does not go online and cannot replace the source check. Do not run it without confirmation. If the monitor reports differences, inspect only each finding's exact JSON review scope and listed source.
