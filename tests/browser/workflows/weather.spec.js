...
test('[WF-WEATHER-001] desktop weather safety alerts restore collapsed details on every page', async ({ page }) => {
  ...
  const expandedForecastBox = await page.locator('.weather-alert__forecast').boundingBox();
  const expandedGuidanceBox = await page.locator('.weather-alert__guidance').boundingBox();
  const expandedUpdatedBox = await page.locator('.weather-alert__updated').boundingBox();
  ...
  expect(expandedForecastBox.x - (expandedTitleBox.x + expandedTitleBox.width)).toBeGreaterThanOrEqual(32);
  expect(expandedUpdatedBox.y + expandedUpdatedBox.height / 2).toBeCloseTo(expandedGuidanceBox.y + expandedGuidanceBox.height / 2, 1);
  expect(expandedUpdatedBox.x).toBeGreaterThan(expandedGuidanceBox.x + expandedGuidanceBox.width);
  expect(collapsedToggleBox.width).toBeGreaterThanOrEqual(44);
  expect(collapsedToggleBox.height).toBeGreaterThanOrEqual(44);
  expect(expandedToggleBox.height).toBe(collapsedToggleBox.height);
  expect(expandedToggleBox.height).toBe(expandedActionBox.height);
  expect(expandedTitleBox.y).toBeCloseTo(collapsedTitleBox.y, 1);
  expect(expandedToggleBox.y).toBeCloseTo(collapsedToggleBox.y, 1);
  expect(expandedActionBox.y).toBeCloseTo(expandedTitleBox.y, 1);
