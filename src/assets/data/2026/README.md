...
**Last completed official-source check:** 2026-06-13 at 9:01 PM EDT. A directly supplied Long Reach host-team manager communication was reviewed for the recurring Kendall Ridge guidance represented by the application. The most recent comprehensive live web review was completed earlier that day at 9:03 AM EDT and accounted for all 135 expected official sources and application-used destinations: the monitor reached 76 modeled-evidence sources, 56 uncovered visitor destinations were reached separately, and three application-used destinations retained their explicit availability exception. All 26 retained official PDFs were unchanged, and modeled page evidence remained current. The latest accepted source-review timestamp is published through `OFFICIAL_SOURCE_CHECKED_AT` in `src/js/config/app-config.js`.

**Last accepted application-data update:** 2026-06-13 at 9:01 PM EDT. Long Reach Marlins home-meet guidance now asks families to leave enough time for parking, setup, and swimmer check-in before the prompt 7:00 AM warm-up and identifies check-in under the large canopy next to the baby pool. The timestamp is published through `OFFICIAL_SOURCE_UPDATED_AT` in `src/js/config/app-config.js` and changes only when reviewed official evidence updates modeled data or an application-used source destination.
...
- On 2026-06-10, Masters-only periods at Swansfield and Talbott Springs were classified as `special-event` so they are not presented as public-use openings. Shared Masters and adult-lap periods at Stevens Forest remain public. Practice-only periods remain explicitly classified as non-public special schedules through `practice-only`.
