...
**Last completed official-source check:** 2026-06-11 at 6:08 PM EDT. A comprehensive live review accounted for all 135 expected official sources and application-used destinations: the monitor reached 76 modeled-evidence sources, 56 uncovered visitor destinations were reached separately, and three application-used destinations retained their explicit availability exception. All 26 retained official PDFs were unchanged, modeled page evidence remained current, and no represented application data or source destination changed. The timestamp is published through `OFFICIAL_SOURCE_CHECKED_AT` in `src/js/config/app-config.js`.
...
- CHS, Harper's Choice, Long Reach, and Wilde Lake team records include structured recurring guidance for their Swansfield, Longfellow, Kendall Ridge, and Faulkner Ridge home meets. Role-specific facts are limited to the audience represented by each source communication, and no meet-specific operational guidance is stored in the meet schedule.
