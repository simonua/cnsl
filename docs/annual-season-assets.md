...
- Confirm every pool has a July 4 `replace-day` override matching `INDEPENDENCE_DAY_NOTICE_SCHEDULE`, and confirm the attention notice starts one week ahead and expires when Independence Day ends. If the official schedules are not uniform, change or disable the notice before activation.
