...
During the active season, `.github/workflows/season-data-monitor.yml` runs `scripts/season-data-agent.js` daily at 05:17 UTC during May, June, and July only. The monitor byte-compares retained official PDFs, recognizes repeatedly unavailable monitored PDF links, and fingerprints relevant content from referenced public pool, staff, recorded practice-schedule, and CNSL publication pages. Publication and schedule-index fingerprints retain PDF destinations so a changed visitor-facing official-source link can be reviewed. Confirmed differences receive a stable candidate key and are delegated once through an issue-free Copilot Agent Tasks API session following the `season-data-reviewer` instructions. The agent compares official evidence with modeled annual JSON and opens a pull request only when application data or an application-used source destination materially changes; it includes updated JSON, supporting retained documents when applicable, accepted source-check metadata, and the reviewed baseline. Non-material website or PDF representation changes create no issue or pull request, and the stable candidate key prevents repeat delegation.

At the first May-to-July scheduled run for a new calendar year, the workflow compares that year to the configured active `YEAR`. If the new year is not active yet, it does not monitor or edit the previous annual folder. It starts an issue-free rollover preparation task constrained to official sources and `src/assets/data/<new-year>/`; for example, the first eligible 2027 run prepares `src/assets/data/2027/` and must leave `src/assets/data/2026/` unchanged. Active rollover work and open rollover pull requests suppress duplicates, but later scheduled runs can resume preparation after a partial pull request merges without activating `YEAR`. Monitoring begins only after required new-year data is reviewed and `YEAR` is activated.
