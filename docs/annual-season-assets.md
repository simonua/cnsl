...
During the active season, `.github/workflows/season-data-monitor.yml` runs `scripts/season-data-agent.js` nightly. The monitor byte-compares retained official PDFs and fingerprints relevant visible text from referenced public pool, staff, recorded practice-schedule, and CNSL publication pages, matching pages by stable data roles so a GoMotion URL relocation without changed published content is ignored. Team home news and event-calendar pages are not fingerprinted because those changing items are not represented in application team data, unless a recorded practice source is hosted on a home page. Individual Columbia Association facility pages are fingerprinted only from their data-bearing facility section, and the Columbia Association schedule index is fingerprinted only for its outdoor-pool schedule section and links. It confirms a changed page on a second request before reporting it. When official content changes or a monitored data page is added or removed, it opens a pull request with refreshed official documents where applicable, updated fingerprints, and a checklist for reviewing and transcribing affected JSON fields, including coach, manager, and practice-schedule information in `teams/teams.json`.
