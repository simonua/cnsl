...
The seasonal source monitor workflow is currently disabled to avoid automatically creating review issues. If explicitly re-enabled during an active season, `.github/workflows/season-data-monitor.yml` runs `scripts/season-data-agent.js` against official sources, byte-compares retained official PDFs, recognizes repeatedly unavailable monitored PDF links, and fingerprints relevant content from referenced public pool, staff, recorded practice-schedule, and CNSL publication pages. Publication and schedule-index fingerprints retain PDF destinations so a changed visitor-facing official-source link can be reviewed. Confirmed differences receive a stable candidate key and create one tracking issue; that issue can be reviewed directly or assigned to Copilot manually using the `season-data-reviewer` instructions. A reviewer opens a pull request only when application data or an application-used source destination materially changes; it includes updated JSON, supporting retained documents when applicable, accepted source-check metadata, and the reviewed baseline. The stable candidate key prevents repeat issue creation for the same observation.
