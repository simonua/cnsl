...
- Store each pool schedule PDF at `pool-schedules/<pool-id>/<YYYY-MM-DD>/<official-filename>`, using the stable annual pool ID and actual download date. When a PDF changes, add the newer artifact under a new date and retain every earlier version.
...
- Treat official public Columbia Association pages and documents as the authoritative source for pool data, and official public CNSL publication and team pages as the authoritative source for meet and team data. Do not transcribe social media or other community-shared content unless it is reflected in an official public source. A dated direct observation by the repository maintainer may add an objective, visible physical pool amenity through `featureOverrides` only after current relevant CA sources have been checked and none explicitly contradicts it; it cannot establish absence, operating status, schedules, admission rules, or accessibility and safety claims.
- For `pools[].features`, use each pool's official Columbia Association pool-location page as the published facility baseline. Review both its narrative description and Amenities list, retain its URL in `caUrl`, normalize terms into clear feature labels, and record the verification date in the annual README. Keep an evidenced correction in `featureOverrides` so the model can derive effective visitor-facing features without obscuring the published baseline.
...
3. Download official PDFs into the correct domain directory, retaining stable, legible filenames. Store each pool PDF as `pool-schedules/<pool-id>/<YYYY-MM-DD>/<official-filename>` and retain earlier dated versions when a publisher regenerates the document. Retain both CNSL practice schedules and team-assignment PDFs when they are published.
...
5. Transcribe data into `<domain>/<domain>.json`; do not copy schedule contents forward without checking the current year's source documents. Stable identifiers and destination URL patterns may be retained when still valid, while dates, matchups, team pool assignments, and schedule content must come from current sources. Harvest pool schedules through the canonical `Pool Schedule Harvesting` checklist in `.github/instructions/data.instructions.md`, including source footnotes, lane allocations, partial-facility conditions, one-date exceptions, cancellations, and exact event identity. Then match every activity and exact combined array to [Pool Activity Access Classification](pool-activity-classification.md), verify whether general use remains available, and update the matrix and schema before accepting a new classification. When an official Time Trials label uses `returning/experienced`, transcribe that qualifier as `returning / experienced` so the visitor-facing label can wrap cleanly on narrow screens without changing its meaning.
6. For pools, re-check every official `caUrl` page before carrying feature values into the new season. Transcribe the published description and Amenities information into `features`, update `FeatureType` for newly published amenity categories, and state the verification date and normalization choices in the annual README. Re-evaluate every `featureOverrides` entry against current evidence and do not copy it forward automatically.
7. Verify every pool's official schedule for Independence Day. The annual data validator requires a July 4 `replace-day` override with public hours matching `INDEPENDENCE_DAY_NOTICE_SCHEDULE` in `src/js/config/app-config.js`. Keep the attention notice scheduled to begin one week ahead, configured as `information`, and set not to show update metadata. Change or disable it if current official data does not support the same hours at every pool. Record the evidence and decision in the annual README.
...
10. Update the annual README with official download links, stored-file inventory, feature verification date, transcription scope, and any data intentionally left in the source PDF because the application schema does not expose it.
...
- Confirm all expected official PDFs exist in their annual/domain document folders. For pools, confirm every stable pool ID has at least one dated artifact and the newest dated folder contains the current official filename.
...
- Confirm every pool feature override is unique, changes the published baseline, records dated evidence, and uses official-source evidence for removals. Confirm no prior-season override was carried forward without renewed evidence.
...
- Confirm every current pool schedule was reviewed through the canonical harvesting checklist, every source annotation was resolved, and the modeled record preserves applicable lane allocations, partial-facility conditions, date-specific exceptions, cancellations, and event identity.
- Confirm every pool has a July 4 `replace-day` override matching `INDEPENDENCE_DAY_NOTICE_SCHEDULE`, and confirm the informational attention notice starts one week ahead, does not show update metadata, and expires when Independence Day ends. If the official schedules are not uniform, change or disable the notice before activation.
...
The seasonal source monitor workflow runs nightly at 05:17 UTC during May, June, and July and can also be dispatched manually. It runs `scripts/season-data-agent.js` against official sources using deterministic requests, hashes, and fingerprints. Its reviewed baseline stores each retained official PDF's SHA-256 digest and available `ETag`, `Last-Modified`, and `Content-Length` response metadata. Normal checks send conditional requests so unchanged PDFs can return `304 Not Modified` without retransferring document bytes; when a validator is unavailable or the publisher returns `200 OK`, the monitor downloads and hashes the PDF before deciding whether content changed. The script also recognizes repeatedly unavailable monitored PDF links and fingerprints relevant content from referenced public pool, staff, recorded practice-schedule, and CNSL publication pages. Publication and schedule-index fingerprints retain PDF destinations so a changed visitor-facing official-source link can be reviewed. Confirmed differences receive a stable candidate key and create or reuse one tracking issue; that issue can be reviewed directly or assigned to Copilot manually using the `season-data-reviewer` instructions. Metadata-only baseline changes may produce a narrowly bounded pull request that cannot include annual data or visitor-facing files. The reviewer defaults to `runlocal` mode, preparing verified edits and validation results without creating a branch, committing, pushing, or opening a pull request. Only an explicit `publish` request may publish a material application-data or application-used source-destination change together with supporting retained documents when applicable, accepted source-check metadata, and the reviewed baseline. The stable candidate key prevents repeat issue creation for the same observation.

At the first May-to-July scheduled run for a new calendar year, the workflow compares that year to the configured active `YEAR`. If the new year is not active yet, it does not monitor or edit the previous annual folder. It creates a rollover preparation tracking issue constrained to official sources and `src/assets/data/<new-year>/`; for example, the first eligible 2027 run prepares `src/assets/data/2027/` and must leave `src/assets/data/2026/` unchanged. An open rollover issue suppresses duplicates, but later scheduled runs can resume preparation after the issue closes without activating `YEAR`. Monitoring begins only after required new-year data is reviewed and `YEAR` is activated.
